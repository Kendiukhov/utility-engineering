...
This note explains, in plain language, what we learned by running the scripted
...
1. **Match the values to the situation.** Strategies that repeat the scenario’s
   vocabulary (ranked list, safety reminders, checklist) keep the model on
   track, whereas mismatched value sets pull it away from the target ordering.
2. **Make the model speak the values out loud.** The more we require the
   assistant to name the values in its own words, the smaller the gap between
   stated and revealed preferences becomes.
3. **Soft audits help but are not enough on their own.** Self-critique improves
   transparency, yet without specific prompts to mention every value it leaves
   gaps that can reappear under value conflicts.
